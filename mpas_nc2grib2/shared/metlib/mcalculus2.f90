...
!   20130719 SHSF : Including Fourth-order Accurate Formuladas
    implicit none
    private
    ...
    !{Rotinas publicas deste modulo 
      ...
      public f0_mcalculus
      public dx,cx,dy,xmax,ymax,scheme
    !}
    !{
     interface KDivV
    ...
    !}
     ...
     !{ Public variables
     integer          :: scheme           ! 0 = second-order-centered or First-order (in case of  domain borders)
                                          ! 1 = Fourth-order 
     integer          ::xmax,ymax         !
     integer          ::cx                ! 0 = circular/global domain; 1 = regional domanin
     Real,allocatable ::dx(:)             !Espacamento da grade X (variavel na direcao y)
     Real             ::dy                !Espacamento da grade Y (fixa) 
     !}
contains
...
!--------------------------------------------------------------------------------------------------    
  function integrall(fx,x,i1,in,undef,err,positive);real::integrall
...
!   utilizado o metodo da area dos trapezios adaptado para valores indefinidos
!--------------------------------------------------------------------------------------------------    
  function integralt(ft,dt,i1,in,undef,err,positive);real::integralt
!{ variaveis  da interface    
...
!-------------------------------------------------------------------------------- 
  subroutine pbase_ptop (p,i1,in,undef,pbase,ptop)
!{ variaveis  da interface    
...
!--------------------------------------------------------------------------------------------------
!-------------------------------------------------------------------------------------------------- 
function isundef  (p,i1,in,undef); logical::isundef
!{ variaveis  da interface    
	real,dimension(:),intent(in)::p  ! valores de niveis de pressao 
	integer,intent(in)::i1  ! Indice inicial da integracao
	integer,intent(in)::in  ! Indice final da integraca
...
!   utilizado o metodo da area dos trapezios adaptado para valores indefinidos
!--------------------------------------------------------------------------------------------------    
  function integrall2(fx,x,i1,in,undef);real::integrall2
!{ variaveis  da interface    
	real,dimension(:),intent(in)::x  ! valores x(i) da funcao F(x(i)) 
	real,dimension(:),intent(in)::fx ! funcao F(x(i)) a ser integrada entre i1 a in
	integer,intent(in)::i1  ! Indice inicial da integracao
...
!
!--------------------------------------------------------------------------------------------------    
  function integral_ab(f,p,a,b,undef);real::integral_ab
!{ variaveis  da interface    
...
!-------------------------------------------------------------------------------
  subroutine KDivV1(K,VX,VY,undef,KGV)
...
!{ Processamento principal de KdivV
	...
	do j=1,jmax 
        ...
        !       if((VX(i2,j2)==undef).or.(vx(i1,j1)==undef).or.(vy(i1,j1)==undef).or.(vy(i2,j2)==undef)) then
			...
			ddx=(VX(i2,j)-Vx(i1,j))/(n1*Dx(j)) 
                        ddy=(VY(i,j2)-Vy(i,J1))/(n2*Dy)
			KGV(i,j)=k*(ddx+ddy)
if ((i==3).and.(j==50).and.(mcalculus_log==1))then 
print *," Div: i,j",i,j
print *," i1,i2,j,U(i1),u(i2)=",i1,i2,j, VX(i1,j),Vx(i2,j)
print *," j1,j2,i,v(j1),u(j2)=",j1,j2,i,Vy(i,j1),Vy(i,j2)
print *,"du,dx,dv,dy,kgv=",i,j,(VX(i2,j)-Vx(i1,j)),(n1*Dx(j)) ,(VY(i,j2)-Vy(i,J1)),(n2*Dy),kgv(i,j)
end if
!stop
		end if
...
!-------------------------------------------------------------------------------
  subroutine KDivV2(K,VX,VY,undef,KGV)
...
!------------------------------------------------------------------------------
subroutine GeostrophicWind(Z,dx,dy,cx,lat,undef,Ug,Vg)
  !{ Variaveis da interface
 ...
 jmax=size(lat,1)
  do j=2,jmax-1
    if (abs(lat(j))<1.0) then
       llat=sign(1.0,lat(j))
       bf2(j)=-abs((sin(lat(j+1)*rad)*dx(j+1)-sin(lat(j-1)*rad)*dx(j-1))/(dx(j)*dy*4.0*omega*sin(llat*rad)**2))
       !bf2(j)=(sin(lat(j+1)*rad)-sin(lat(j-1)*rad))/(dy*4.0*omega*sin(llat*rad)**2)
    else
       bf2(j)=-abs((sin(lat(j+1)*rad)*dx(j+1)-sin(lat(j-1)*rad)*dx(j-1))/(dx(j)*dy*4.0*omega*sin(lat(j)*rad)**2))
...
subroutine fGeostrophicWind(Z,dx,dy,cx,undef,fUg,fVg)
  !{ Variaveis da interface
   real,dimension(:,:),intent(in) ::Z       !Alturas geopotenciais (metros)
   Real,dimension(:),  intent(in) ::dx      !Espacamento da grade X (variavel na direcao y)
   Real,               intent(in) ::dy      !Espacamento da grade Y (fixa)
...
!------------------------------------------------------------------------------
subroutine ThermalWind(T,P,dx,dy,cx,lat,undef,Ut,Vt)
  !{ Variaveis da interface
         ...
         if (abs(lat(j))>=5.0) then
          ...
          do i=1,imax
             ...
             if(undefr(i,j)==0) then !(Z(i2,j)==undef).or.(Z(i1,j)==undef).or.(Z(i,j1)==undef).or.(Z(i,j2)==undef)) then
                 ...
                 dtdy=(tm(i,j2,k)-tm(i,j1,k))/(n2*dy)
...
!-------------------------------------------------------------------------------
!Advec2D! Calcula  Va(Nabla Vb)i e Va(nabla vb)j                          | SHSF
!------------------------------------------------------------------------------
! Calcula a Adveccao horizontal de Vb por Va  
!
...
!------------------------------------------------------------------------------
! Utilizacao:
!   1) Fornece Va e Vb de dimensoes (i,j,k,e1)  onde :
!     i,j,k sao as coordendas do campo 
!     e1 = 1,2 ou 3 identificando as componentes vetorias nas direcoes x,y e z respectivamente
!
...
!     a)  i,j,k tem o significado usual
!     b) e1 =1,2 representa o resultado de  Va(Nabla Vb)i e Va(nabla vb)j 
!
!     Ou seja 
!
...
subroutine Advec2D(va,vb,dx,dy,P,cx,undef,va_Dvb)
  !{ Variaveis da interface
...
!{ Caldula dx,dy,dp considerando valores indefinidos  ao redor 
   do e2=1,3
      ...
      elseif ((e2==3).and.(p(i,j,k2)/=undef).and.(p(i,j,k1)/=undef))  then
        ...
        if (d(i,j,k,3)>=0) then
            print *,":MCALCULOS:Advec2D: dP positiv! Nivel/dp/p2/p/p1",k,d(i,j,k,3),p(i,j,k2),p(i,j,k),p(i,j,k1)
	    stop 
        endif
      else
        d(i,j,k,3)=0
       ...
       !{ Calculando termos isoladamente
        do e2=1,3
          ...
          do e1=1,2
            ...
            !{ Se a derivada e na direcao Y (e2=2) entao aplica o calculo da compensacao da curvatura
            !  Caso contrario procede a derivacao normalmente 
            if (e2==2) then 
              DVi=(Vb(i2,j2,k2,e1)-Vb(i1,j1,k1,e1))
              Dxj= d(i,j,k,e2)
            else
              DVi=(Vb(i2,j2,k2,e1)-Vb(i1,j1,k1,e1))
              Dxj= d(i,j,k,e2)
            ...
            if (dxj/=0.0) then 
              Vj_DviDxj(e1,e2)=Va(i,j,k,e2)*DVi/Dxj	
            else
              Vj_DviDxj(e1,e2)=undef
            end if
            IF (overflow_err(Vj_DviDxj(e1,e2),undef)) then
...
subroutine Advec2DE(Va,E,dx,dy,cx,undef,va_DE)
  !{ Variaveis da interface
       ...
       !{ Calculando termos isoladamente
        do e2=1,2
          ...
          if (dxj/=0.0) then
            ...
            IF (overflow_err(Vj_DeDxj(e2),undef)) then
...
!------------------------------------------------------------------------------!
subroutine advec2DEvh(u,v,e,dx,dy,cx,undef,advec_e)
  !{ Interface
  ...
  !}  
    ...
    do k=1,kmax
      call grad2D(1.0,e(:,:,k),dx,dy,cx,undef,gradE(:,:,:))
...
!------------------------------------------------
subroutine advec3DP(u,v,w,e,dx,dy,p,cx,undef,adh,adv)
  !{ Interface
    real,dimension(:,:,:),intent(in) :: u     !Componente zonal da vento u(i,j,k)
    real,dimension(:,:,:),intent(in) :: v     !Componente meridional do vento v(i,j,k)
  ...
  !}  
    ...
    call grad3DP(E,dx,dy,P,cx,undef,gradE(:,:,:,:))
...
subroutine grad3D(F,dx,dy,dz,cx,undef,gradF)
!{ Interface 
   real,dimension(:,:,:),intent(in)::F ! matriz do campos escalar
   real,dimension(:),    intent(in)::dx !matriz do espacamento da grade em X
   real,                 intent(in)::dy !espacamento da grade y
   real,                 intent(in)::dz !
    ...
    do j=1,jmax
      do k=1,kmax
        ...
        if((F(i,j2,k)==undef).or.(F(i,j1,k)==undef)) then
          ...
          gradf(i,j,k,2)=(F(i,j2,k)-F(i,j1,k))/(n2*dy) !.... Gradiente na direcao y
        end if
...
!
subroutine grad3DP(F,dx,dy,p,cx,undef,gradF)
!{ Interface 
   real,dimension(:,:,:),  intent(in)::F ! matriz do campos escalar
   real,dimension(:),      intent(in)::dx !matriz do espacamento da grade em X
   real,                   intent(in)::dy !espacamento da grade y
...
subroutine grad2D(g,F,dx,dy,cx,undef,gradF)
  !{ Interface 
   real,                 intent(in) ::g     ! uma constante de multiplicacao
   real,dimension(:,:),  intent(in) ::F     ! matriz do campos escalar F(i,j)
   real,dimension(:),    intent(in) ::dx    ! Vetor com os intervalos de longitude dx(j) em funcao da latitude
   real,                 intent(in) ::dy    ! Intervalos de latitude 
   integer,              intent(in) ::cx    ! Se 0 = Modelo global (grade circular em x) se 1 = Modelo regional
   real,                 intent(in) ::undef !.Valor indefinido
 ...
 !{ Calculo do Gradiente 
    do i=1,imax
      do j=1,jmax
        call get_index(i,imax,cx_in,i1,i2,n1)
        call get_index(j,jmax,1,j1,j2,n2)
        if((F(i2,j)==undef).or.(F(i1,j)==undef)) then
          gradf(i,j,1)=undef
        else
          gradf(i,j,1)=g*(F(i2,j)-F(i1,j))/(n1*dx(j))!....... Gradiente na direcao X
        end if

        if((F(i,j2)==undef).or.(F(i,j1)==undef)) then
          gradf(i,j,2)=undef
        else
          gradf(i,j,2)=g*(F(i,j2)-F(i,j1))/(n2*dy) !.... Gradiente na direcao y
        end if
      end do !j
    end do !I
...
subroutine dfdx(F,dx,undef,ddx)
...
!
!------------------------------------------------------------------------------
subroutine dfdt(F,dt,undef,ddt)
  !{ interface
...
!
!-------------------------------------------------------------------------------
	function  incx(i,inc)_index(i,imax,cx,i1,i2,n)
	!{Variaveis da interface
		integer,intent(in)::i    ! cuIndice fornecido
...
function average(value,i1,i2,undef); real::average
!{Variaveis da interface
	real,dimension(:)::value  !Valores para calculo da media 
...
!}
	...
	do i=i1,i2
			...
			average=(average*real(ct)+value(i))/real(ct+1)
			ct=ct+1
		    end if
	end do

	if (ct==0)average=undef
...
subroutine movaverage(value,i1,i2,di,undef,fvalue)
!{Variaveis da interface
...
function mult(a,b,undef);real::mult
...
!--------------------------------------------------
function overflow_err(v,undef); logical::overflow_err
  !{ Variaveis da interface
  real,intent(in)::v
  real,intent(in)::undef
  !}
